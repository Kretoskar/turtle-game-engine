...
void Turtle::Scene::Init(ECS& ecs)
{
	...
	TransformComponent& t = ecs.AddComponent<TransformComponent>(arrow);

	MeshComponent& m = ecs.AddComponent<MeshComponent>(arrow);
	m.Mesh = Turtle::Engine::AssetsRegistry->ArrowMesh.get();
}